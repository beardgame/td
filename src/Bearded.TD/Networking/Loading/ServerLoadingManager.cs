...
namespace Bearded.TD.Networking.Loading
{
    class ServerLoadingManager : LoadingManager
    {
        ...
        private void debug_sendBlueprints()
        {
            ...
            Dispatcher.RunOnlyOnServer(() => SendUnitBlueprint.Command(Game, new UnitBlueprint(
                Game.Ids.GetNext<UnitBlueprint>(), "strong", 250, 20, 1.5.S(), new Speed(1.2f), 3f, Color.Yellow)));
            Dispatcher.RunOnlyOnServer(() => SendUnitBlueprint.Command(Game, new UnitBlueprint(
                Game.Ids.GetNext<UnitBlueprint>(), "fast", 50, 4, .5.S(), new Speed(3), 3f, Color.CornflowerBlue)));
            Dispatcher.RunOnlyOnServer(() => SendUnitBlueprint.Command(Game, new UnitBlueprint(
                Game.Ids.GetNext<UnitBlueprint>(), "tank", 1000, 50, 2.S(), new Speed(.8f), 10f, Color.SandyBrown)));
        }
    }
}
